...
was the receival of data from the controller area network bus (CAN bus). The 
implemented solution uses an advanced system to ensure that no transmissions of 
data packets are missed. 
Each time data is transmitted on the CAN bus the MBED board receives an interrput. 
...
Following the receival of text packets, a checksum is sent. 
If the length of the text does not match the checksum, the text is ignored. 
Hence, this ensures all packets were successfully received, and that none have 
been corrupted. 
If the checksum value is correct, and the text packets have been correctly 
